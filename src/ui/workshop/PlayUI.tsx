...
export function PlayUI({ instrument, forcedType, onPlay }: PlayUIProps) {
  ...
  if (type === 'drag') {
    return <div className="play-surface drag-surface" onPointerMove={(event) => event.buttons === 1 && onPlay(notes[Math.floor((event.nativeEvent.offsetY / Math.max(1, event.currentTarget.clientHeight)) * notes.length)] ?? notes[0], '16n', 0.55)} />;
  }
  if (type === 'bow') {
